...
public class IcrListPanelUiTest extends AssertJSwingJUnitTestCase {
  ...
  public void showsTable() {
    JFrame frame =
        UiTestSupport.showFrame(
            f -> {
              f.add(new IcrListPanel(UiTestSupport.testApiClient(), f));
            });

    FrameFixture window = new FrameFixture(robot(), frame);
    UiTestSupport.dismissBlockingDialogs(robot());
    window.requireVisible();
    window.button(withText("Submit New")).requireVisible();
    window.cleanUp();
  }
}
