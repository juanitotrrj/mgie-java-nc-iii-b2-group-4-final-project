...
public class AdminDashboardUiTest extends AssertJSwingJUnitTestCase {
  ...
  public void showsMetricCards() {
    JFrame frame =
        UiTestSupport.showFrame(
            f -> {
              f.add(new AdminDashboard(UiTestSupport.testDashboardApi()));
            });

    FrameFixture window = new FrameFixture(robot(), frame);
    UiTestSupport.dismissBlockingDialogs(robot());
    window.requireVisible();
    window.label(withText("Total Products")).requireVisible();
    window.label(withText("Total Users")).requireVisible();
    window.cleanUp();
  }
}
