...
public class SaleHandlerTest {
  ...
  private final SaleHandler handler = new SaleHandler();

  @Test(expected = ApiException.class)
  public void handle_unknownSubpath_throws404() throws IOException {
    RequestContext ctx =
        HandlerTestSupport.context("GET", "/api/sales/unknown", null, new HashSet<String>());
    handler.handle(ctx);
  }
  ...
  public void handleGet_withoutPermission_throwsForbidden() throws IOException {
    RequestContext ctx =
        HandlerTestSupport.context("GET", "/api/sales", null, new HashSet<String>());
    handler.handle(ctx);
  }
  ...
  public void handlePost_withoutPermission_throwsForbidden() throws IOException {
    RequestContext ctx =
        HandlerTestSupport.context("POST", "/api/sales", "{}", new HashSet<String>());
    handler.handle(ctx);
  }
}
